...
export class RetentionService {
  ...
  async run(now: Date = new Date()): Promise<RetentionReport> {
    const days = effectiveRetention((await new SettingsService(this.db).deployment()).retention);
    ...
    if (Object.values(report).some((n) => n > 0)) {
      await this.db.transaction((tx) =>
        recordAudit(tx, systemActor('retention', `retention:${now.toISOString()}`, 'Retention policy'), {
          ...
          summary: `Retention applied: ${Object.entries(report).filter(([, n]) => n > 0).map(([k, n]) => `${k} ${n}`).join(', ')}`,
          after: { report, days },
        }),
      );
    }

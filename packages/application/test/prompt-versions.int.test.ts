...
let agentId: string;
const person = (role: Principal['role'], name: string): ActorContext => ({ principal: { userId: uuidv7(), role, displayName: name, teamIds: [], via: 'UI' }, correlationId: 'c' });
...
beforeAll(async () => {
  ...
  agentId = (await new AgentService(t.db).create(lead, { name: 'Maya', purpose: 'customer support', conversationType: 'SUPPORT', description: '' })).id;
});
afterAll(async () => {
  await t?.drop();
});
...
describe('prompt versioning (docs/05 §2)', () => {
  it('starts every agent on an immutable, activated v1', async () => {
    const versions = await prompts.versions(agentId);
  ...
  it('versions a draft with author, reason and the changed components only', async () => {
    ...
    expect((await prompts.draft(agentId)).dirty).toBe(true);
    ...
    expect((await prompts.draft(agentId)).dirty).toBe(false);
  });
  ...
  it('activates, invalidates the agent cache generation, audits, and rolls back to an older version', async () => {
    const [v2, v1] = await prompts.versions(agentId);
    ...
    expect(await prompts.diff(agentId, v1!.id, v2!.id)).toEqual([{ key: 'behavior', before: expect.any(String), after: 'Keep replies under 60 words.' }]);

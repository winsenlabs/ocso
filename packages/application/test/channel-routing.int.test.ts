...
describe('channel routing', () => {
  it('attaching an agent to a channel with no default agent makes it the default', async () => {
    ...
    // A channel that already routes somewhere keeps its default.
  ...
  it('without a default agent, a channel attached to exactly one agent still routes to it', async () => {
    ...
    // Two attached agents and no default: ambiguous, so still rejected.
    await t.db.insert(agentChannels).values({ agentId: ava, channelId: id });
    const rejected = await ingress.receive(id, msg('wamid.route.2', '+919800000002'), 'c2');

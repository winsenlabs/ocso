...
- Prompt compilation, component versioning and cache breakpoints: `packages/prompt-compiler`; versions are immutable once activated (database trigger), activation bumps the agent's cache generation (`cache_generations`), which invalidates the worker's hot turn cache (PM/ARCHITECTURE-DECISIONS.md ADR-006, ADR-009).

...
export interface ChannelKindDescriptor {
  ...
  setupGuide?: readonly ChannelSetupStep[];    // the step-by-step checklist the channel dialog shows (below)
  troubleshooting?: readonly ChannelTroubleshooting[]; // { id, problem, fix }; connection checks link to an id
  setupFiles?: readonly ChannelSetupFile[];    // app manifests / packages; only {{webhookUrl}}, {{webhookHost}}, {{settings.<key>}} are filled
  setupSteps?: readonly string[];              // deprecated: plain sentences, served as one guide step each
...
`messageTemplates` (it implements `listTemplates`, `createTemplate` and `sendTemplate`). It always
carries `setupGuide` (built from `setupSteps` for kinds that only have those) and `troubleshooting`.

**The setup guide.** Each `ChannelSetupStep` is plain text (no markup is rendered):

```ts
interface ChannelSetupStep {
  title: string;                                    // up to 120 characters
  body: string;                                     // up to 2000
  items?: string[];                                 // bullet points
  table?: { head: [string, string]; rows: [string, string][] }; // e.g. each scope and why
  values?: { label: string; value: string }[];      // shown with Copy; may use the setup-file placeholders
  links?: { label: string; href: string }[];        // https only
  files?: string[];                                 // setupFiles keys offered in this step
  form?: boolean;                                   // OCSO's settings and secrets form goes here (at most one step)
  check?: string;                                   // how to tell the step worked
}
```

Registration (and `checkPlugin`) refuses a guide with an empty title, a non-https link, an unknown
placeholder, a `files` key that is not a setup file, two `form` steps or duplicate troubleshooting ids.
A `ConnectionCheck` may set `help` to a troubleshooting `id`; the dialog links the failed check to it.

**Setup files.** A text file (`application/json`, `text/yaml`, `text/plain`) has a `template`. An
`application/zip` file (an app package) has `entries` instead: text templates, and `image/png` icons as
base64. Placeholders are `{{webhookUrl}}`, `{{webhookHost}}` (its host) and `{{settings.<key>}}`;
secrets are never interpolated. `GET /v1/channels/:id/setup-files/:key` (`channels.read`) renders any
file and builds a zip on the server; it answers `400 setup_file_incomplete` while a value it needs is
not saved.
...
beside the setup steps after saving. Every save runs `validateConfig`; on a draft, problems about a
setting or secret that was not given at all are set aside (the admin creates the draft first to get
its webhook URL), and activation checks everything. Problems written as `settings.<path>: message` or
`secrets.<key>: message` show under the matching field. Secrets go to the SecretStore as `CHANNEL_TOKEN` references and are never returned. If you
implement `checkConnection`, the channel gets a **Test connection** button. For webhook kinds the
dialog first saves a draft with just the name, then shows your `setupGuide` as a checklist with the
webhook URL (the API derives `webhookPath` from your segment), your setup files and the form inside
the `form` step, and your `troubleshooting` below it. The channel

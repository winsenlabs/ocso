...
> [!IMPORTANT]
> **Original design spec (2026-09): parts are superseded; see [governance.md](../../concepts/governance.md), [conversations.md](../../concepts/conversations.md), [permissions.md](../../reference/permissions.md).** This page is kept for history. Where it and the code disagree, the code and [PM/ARCHITECTURE-DECISIONS.md](../../../PM/ARCHITECTURE-DECISIONS.md) win. Links inside it may point to pages that have since moved.
...
Service members should be able to see all conversations they are authorized to access, including:
...
When a Service member takes control:
...
Heads and Leads need a productized way to identify bad behavior and improve instructions.
...
- Templates are business content: `message_templates.manage` (Head and Lead, for channels used by their teams' agents; Tech, every channel) drafts them in OCSO (**Message templates**, for channels whose adapter supports templates, e.g. WhatsApp). Submitting a draft to the provider and deleting a template (`message_templates.delete`, Head) are maker–checker proposals checked with `approvals.check.channels` (ADR-030); Service members cannot. The submitter gets an in-app notice when the provider approves, rejects (with the reason), pauses or disables a template.
- **Team-scoped virtual agents (ADR-026).** Agents are owned by teams (`agent_teams`); "Head and Lead: virtual-agent business prompt/instructions" (§6) means *the agents their teams own*. Rules, enforced in the application services (`packages/application/src/agents/access.ts`, `owners.ts`), not only the UI:
  - A Head or Lead reads and manages an agent only as a member of one of its owning teams — settings, status, prompts (draft, versions, diff, preview, activate), escalation rules, tool grants, analytics, reviews, corrections, evaluations and agent-scoped alerts and alert rules. Any other agent answers **404** (never 403), so its existence does not leak.
  ...
  - Tech reads every agent (`agents.read_all`, technical fields) and reassigns owning teams across teams (`agents.assign_owner`, `PUT /v1/agents/:id/owners`, audited as `agent.owners_change`) — e.g. when a Lead leaves. Tech still cannot edit prompts, tools or go-live.
  - A Service member reads (never manages) agents owned by their teams or reachable through their teams' queues (default queue, or an escalation rule targets the queue), for display in the workspace.
  ...
  - Platform-wide escalation rules (agentId null) are listed with every agent and are read-only through the agent routes. Channels no longer name an agent: a channel is attached to a router, which routes to queues and so to agents (ADR-031); the agent's Channels tab shows what reaches it ("Reached through").
  - Existing agents were given the teams serving their default queue as owners by the migration; agents with no such team are unowned (Tech only) until assigned. A Head or Lead in no team manages no agent: the web shows "Join or create a team to create agents".

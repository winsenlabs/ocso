...
> [!NOTE]
> This is a contributor page. These driver kinds are **first-party only**: the `OCSO_PLUGINS` loader
> refuses an installed package that contributes blob, secrets, queue, deployment or audit-store drivers
> (only channels, model providers, alert destinations and email drivers are open to third parties; see
> [Plugins](../concepts/plugins.md)). A new driver lands in this repository, in its package, and is
> registered in `FIRST_PARTY_PLUGINS`.

These five contracts let the same application run on a single Docker host and on AWS without two code
paths (engineering rule [§17](engineering-rules.md#17-same-application-two-deployment-shapes)). Each is a
...
contract and never branches on the driver. [Email senders](../guides/email.md) work the same way.
...
A driver ships in a first-party plugin (see [One composition root](../concepts/architecture.md#one-composition-root)). A hypothetical Google Cloud Storage driver, added to this repository, would look like this:
...
const plugin: OcsoPlugin = { name: '@ocso/blob', blobDrivers: [localBlobDriver, s3BlobDriver, gcsBlobs] }; // in packages/bootstrap/src/first-party.ts
```
...
- Driver-specific settings are declared in `packages/config/src/env.ts` (the parsed env strips
  unknown keys), so a driver with settings of its own reads them from `process.env`.
- Test coverage is uneven. The `ecs` deployment adapter has unit tests with fake AWS clients. The `s3`,
  ...
  [Deploy on AWS](../guides/deploy/aws.md#limits-and-known-gaps).

...
describe('workspace actions per control state (ADR-005, docs/09 §4)', () => {
  it('lets an exec take over an AI conversation but not write to the customer', () => {
    expect(view('CS_EXEC', 'AI_ACTIVE')).toEqual({ actions: ['take-over'], canCompose: false, holdsIt: false, offeredToMe: false });
  });

  it('offers Claim on open pickup and Accept/Decline on an auto-assign offer to me', () => {
    expect(view('CS_EXEC', 'WAITING_FOR_HUMAN').actions).toEqual(['claim']);
    const offered = view('CS_EXEC', 'WAITING_FOR_HUMAN', ME, 'OFFERED');
    ...
    expect(view('CS_EXEC', 'WAITING_FOR_HUMAN', OTHER, 'OFFERED').actions).toEqual([]);
  });

  it('gives the holder return, transfer and resolve plus the composer', () => {
    const held = view('CS_EXEC', 'HUMAN_ACTIVE', ME);
  ...
  it('locks the composer for an exec watching a colleague’s conversation', () => {
    expect(view('CS_EXEC', 'HUMAN_ACTIVE', OTHER)).toMatchObject({ actions: [], canCompose: false });
  });

  it('lets a lead with assign rights act on a conversation someone else holds', () => {
    const lead = view('CS_LEAD', 'HUMAN_ACTIVE', OTHER);
  ...
  it('offers Cancel return while the AI resumes, and Reopen once resolved', () => {
    expect(view('CS_EXEC', 'AI_RESUMING', ME).actions).toEqual(['cancel-return']);
    expect(view('CS_EXEC', 'AI_RESUMING', OTHER).actions).toEqual(['take-over']);
    expect(view('CS_EXEC', 'RESOLVED', ME).actions).toEqual(['reopen']);
  });

  it('gives a Platform Tech Admin nothing (no conversation permissions)', () => {
    for (const state of ['AI_ACTIVE', 'WAITING_FOR_HUMAN', 'HUMAN_ACTIVE', 'RESOLVED'] as const) {
      expect(view('PLATFORM_TECH_ADMIN', state, ME)).toMatchObject({ actions: [], canCompose: false });
    }
  });
});

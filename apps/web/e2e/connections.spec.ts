 ...
 *   OCSO_ENABLE_DEV_PROVIDERS=true E2E_API_PORT=4420 E2E_WEB_PORT=3420 E2E_DB_NAME=ocso_e2e_conn npx playwright test e2e/connections.spec.ts
 */
test.describe.configure({ mode: 'serial' });

const repo = fileURLToPath(new URL('../../../', import.meta.url));
const DEMO_PORT = Number(process.env['E2E_MCP_DEMO_PORT'] ?? E2E.apiPort + 7);
const DEMO_TOKEN = 'e2e-meridian-demo-token-0123456789';
const FAKE_KEY = 'sk-e2e-not-a-real-key-0123456789';
const USERS = {
...
test('Tech Admin adds a webhook (secret shown once) and sees every credential by reference only', async ({ page }) => {
  ...
  const secret = (await created.getByLabel('Signing secret').textContent()) ?? '';

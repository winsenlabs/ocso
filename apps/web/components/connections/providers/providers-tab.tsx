...
import { listPricing, listProfiles, listProviderKinds, listProviders, type Provider } from '@/lib/api/models';
...
export async function ProvidersTab({ session, params }: { session: Session; params: Params }) {
  ...
  const priceInitial =
    kindParam && priceModel
      ? { kind: kindParam, model: priceModel, suggestion: missing?.find((m) => m.providerKind === kindParam && m.model === priceModel)?.catalog ?? null }
      : undefined;
  ...
  return (
    <>
      ...
      {canPricing && (dialog === 'pricing-new' || (dialog === 'pricing-edit' && price)) ? (
        <PricingDialog key={price?.id ?? `new-${kindParam ?? ''}-${priceModel ?? ''}`} kinds={kinds} row={price ?? null} closeHref={closeHref} initial={price ? undefined : priceInitial} />
      ) : null}
    </>
  );
}

...
describe('web chat widget API', () => {
  ...
  it('serves branding, limits and the embedding allowlist without secrets', async () => {
    const res = await h.http().get(`/public/webchat/${key}/config`).expect(200);
  ...
  it('echoes the client message id and joins agent messages to their turn', async () => {
    const empty = await h.http().get(`/public/webchat/${key}/messages`).set(auth(visitor)).expect(200);
    expect(empty.body).toMatchObject({ conversationId: null, agentName: 'Maya', messages: [], notices: [], status: { mode: 'ai' } });

    const upload = await h.http().post(`/public/webchat/${key}/attachments`).set(auth(visitor)).set('content-type', 'image/png').send(PNG).expect(201);
    const sent = await h
      .http()
      .post(`/public/webchat/${key}/messages`)
      .set(auth(visitor))
    ...
    const after = await h.http().get(`/public/webchat/${key}/messages?afterSeq=${mine.seq}`).set(auth(visitor)).expect(200);
  ...
  it('turns control changes into customer-safe notices and status', async () => {
    ...
    await h.http().post(`/public/webchat/${key}/messages`).set(auth(visitor)).send({ clientMessageId: 'cm_widget_0002', text: 'talk to a human' }).expect(201);
    await runTurn(conversationId);
    const waiting = await h.http().get(`/public/webchat/${key}/messages`).set(auth(visitor)).expect(200);
    ...
    const joined = await h.http().get(`/public/webchat/${key}/messages`).set(auth(visitor)).expect(200);
  ...
  it('accepts audio only in allowed formats and enforces per-kind limits', async () => {
    ...
    await h.http().post(`/public/webchat/${key}/attachments`).set(auth(visitor)).set('content-type', 'audio/mpeg').send(mp3).expect(201);
    const txt = await h.http().post(`/public/webchat/${key}/attachments`).set(auth(visitor)).set('content-type', 'application/octet-stream').set('x-ocso-content-type', 'text/plain').send(Buffer.from('order 1234')).expect(201);
    expect(txt.body.mimeType).toBe('text/plain');
    const bad = await h.http().post(`/public/webchat/${key}/attachments`).set(auth(visitor)).set('content-type', 'application/octet-stream').send(Buffer.from('MZ\x90\x00')).expect(400);

...
**Consequences.** A CS Lead in no team manages nothing: the web shows "Join or create a team to create agents", and the Tech Admin must add leads to teams (leads cannot change their own memberships today). Escalation rules are addressed through their own agent (`/agents/:agentId/escalation-rules/:ruleId` 404s for another agent's rule); platform-wide rules are read-only there. Known gaps: the audit log (`audit.read`) is not team-scoped; queue configuration remains shared (not team-owned).

...
- [ ] `pnpm lint` passes (source guards: file size, import boundaries, package cycles, plugin boundary)
- [ ] Tests added or updated, and `pnpm test` / `pnpm test:int` pass
- [ ] Browser flows touched? The relevant `apps/web/e2e` spec passes
- [ ] Schema changed? Hand-written migration plus refreshed `meta/` files (CONTRIBUTING.md); `drizzle-kit generate` not run against `migrations/`; no committed migration edited
- [ ] New kind of configuration? Approval descriptor registered (maker–checker) and every privileged write audited in the same transaction
